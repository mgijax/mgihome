...
def checkConfig():
        ...
        if not os.path.exists(config['SUBMISSION_DIRECTORY']):
                raise error('Invalid directory for SUBMISSION_DIRECTORY: "%s"' % config['SUBMISSION_DIRECTORY'])
...
def main():
        ...
        updateFields(parms)
        log('point 1 - updated fields (%d parms)' % len(parms))
        ...
        if UPLOADED_FILES:
                log ('point 1a - has uploaded files')
                try:
                        log ('point 1a1 - creating directory')
                        myDir = createDirectory()
                        log ('point 1a2 - created directory')
                except:
                ...
                try:
                        log ('point 1a3 - saving files')
                        saveUploadedFiles(myDir)
                        log ('point 1a3 - saved files')
                except:
        ...
        # submitted.
                ...
                log ('point 1b - has submission in parameters')
